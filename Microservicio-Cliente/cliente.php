...
function create_request_food($requested_food){
	...
	$ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://172.10.10.11:4000/order_restaurant");
...
function get_state_restaurant($order){
	...
	$ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://172.10.10.11:4000/state_restaurant");
...
function get_state_dealer($order){
	...
	$ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://172.10.10.11:4000/state_dealer?order=".$order);
    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json'));
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_HEADER, 0);
    $response = curl_exec($ch);
